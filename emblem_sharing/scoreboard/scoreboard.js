...
let appState = {
    ...
    selectedPlayerHasEmblem: false,
    selectedPlayerHasEmblemHidden: false,
};
...
// Custom Emblem Modifications
var custom_emblem_cache = {};

function refresh_local_emblem() {
    if (localStorage.getItem("emblemURL")) {
        console.log("refresh_local_emblem")
        dew.command("Player.PrintUID").then(resp => {
            custom_emblem_cache[resp.split("0x")[1]] = {
                "hidden": false,
                "image": localStorage.getItem("emblemURL"),
            };
            console.log(custom_emblem_cache);
        });
    }
}

dew.on("reload-local-emblem", function (ev) {
  refresh_local_emblem();  
});

dew.on("clear-local-emblem", function (ev) {
    dew.command("Player.PrintUID").then(resp => {
        delete custom_emblem_cache[resp.split("0x")[1]];
    });
});
...
dew.on("emblem-data", function (ev) {
    ...
    if (uid in custom_emblem_cache) {
        custom_emblem_cache[uid].image = ev.data.data;
    } else {
        custom_emblem_cache[uid] = {
            "hidden": false,
            "image": ev.data.data,
        };
    }
    ...
    methods: {
        openPlayerContextMenu(e, player) {
			...
			dew.getSessionInfo().then(((result) => {
                ...
                // Custom Emblem Modifications
                this.selectedPlayerHasEmblem = player.uid in custom_emblem_cache;
                this.selectedPlayerHasEmblemHidden = !(player.uid in custom_emblem_cache) ? false : custom_emblem_cache[player.uid].hidden;
			}));
        },
        ...
        // Custom Emblem Modifications
        hidePlayerEmblem() {
            if (custom_emblem_cache[this.selectedPlayer.uid]) {
                custom_emblem_cache[this.selectedPlayer.uid].hidden = !custom_emblem_cache[this.selectedPlayer.uid].hidden;
                this.selectedPlayerHasEmblemHidden = !this.selectedPlayerHasEmblemHidden;
                dew.notify("hide-player-custom-emblem", { uid: this.selectedPlayer.uid });
            }
        },
